 ...
 */
const NationalInsurance = ({ calcNIDispatch, income, ni, setIncomeDispatch }) => {
  // Batch the actions. First, set the income and then calculate the NI.
  const onChange = ev => {
    setIncomeDispatch(parseFloat(ev.target.value));
    calcNIDispatch();
  };
  ...
  return (
    ...
    >
      ...
      <p
        ...
        `}
      >
        Compare your contributions from this tax year to last tax year.
      </p>
      <main
        css={css`
          display: grid;
...
const mapDispatchToProps = {
  calcNIDispatch: calcNI,
  setIncomeDispatch: setIncome
};

NationalInsurance.propTypes = {
  calcNIDispatch: PropTypes.func.isRequired,
  ni: PropTypes.shape({
  ...
  setIncomeDispatch: PropTypes.func.isRequired
};
